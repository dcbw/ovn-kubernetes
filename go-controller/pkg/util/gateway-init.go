...
func ensureGatewayPortAddress(portName string) (string, *net.IPNet, error) {
	...
	// Grab the 'join' switch prefix to add to our gateway router's IP
	joinSubnet, stderr, err := RunOVNNbctl("--if-exists", "get",
		"logical_switch", "join", "other-config:subnet")
	if err != nil {
		return "", nil, fmt.Errorf("Failed to get 'join' switch subnet: "+
			"%s, stderr: %q, %v", joinSubnet, stderr, err)
	}
	if joinSubnet == "" {
		return "", nil, fmt.Errorf("Empty 'join' switch subnet")
	}
	joinSubnetSplit := strings.Split(strings.Trim(joinSubnet, `"`), "/")
	if len(joinSubnetSplit) != 2 {
		return "", nil, fmt.Errorf("Invalid 'join' switch subnet %q", joinSubnet)
	}
	stringCIDR := routerIP + "/" + joinSubnetSplit[1]
...
func GatewayInit(clusterIPSubnet []string, nodeName, nicIP, physicalInterface,
	...
	stdout, stderr, err = RunOVNNbctl(
		...
		"--", "lsp-set-addresses", gwSwitchPort, "router",
