	...
	It("sets correct OVN external IDs", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			fakeCmds := ovntest.AddFakeCmd(nil, &ovntest.ExpectedCmd{
				Cmd: "ovs-vsctl --timeout=5 set Open_vSwitch . external_ids:ovn-encap-type=geneve external_ids:ovn-encap-ip=" + nodeName + " external_ids:k8s-api-server=\"" + apiserver + "\" external_ids:k8s-api-token=\"" + token + "\" external_ids:k8s-ca-certificate=\"" + cacert + "\"",
			})
