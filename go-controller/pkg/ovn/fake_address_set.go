...
func (f *fakeAddressSetFactory) WithAddressSet(name string, doFn AddressSetDoFunc) error {
	if as := f.getAddressSet(name); as != nil {
		defer as.Unlock()
		return doFn(as)
	}
	return nil
...
func (f *fakeAddressSetFactory) DeleteAddressSet(name string) {
	if as := f.getAddressSet(name); as != nil {
		defer as.Unlock()
		as.destroyUnlocked()
	}
...
func (as *fakeAddressSet) replaceIPs(ips []net.IP) error {
	Expect(as.destroyed).To(BeFalse())
...
func (as *fakeAddressSet) AddIPUnlocked(ip net.IP) error {
	Expect(as.destroyed).To(BeFalse())
	ipStr := ip.String()
...
func (as *fakeAddressSet) DeleteIPUnlocked(ip net.IP) error {
	Expect(as.destroyed).To(BeFalse())
	ipStr := ip.String()
	if _, ok := as.ips[ipStr]; ok {
		delete(as.ips, ipStr)
...
func (as *fakeAddressSet) destroyUnlocked() {
	Expect(as.destroyed).To(BeFalse())
	as.destroyed = true
	as.removeFn(as.name)
}
